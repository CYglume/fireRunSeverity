...
######################################################################################
# Algorithm                                                                          #
# Run Algorithm to get VPD data from ERA5-Land hourly dataset                        #
# Refer to API link: https://cds.climate.copernicus.eu/datasets/reanalysis-era5-land #
######################################################################################

for (AreaName in AreaList){
  VPD_tbl_path <- file.path(run_DataDir,
                             AreaName, "input", "VPD.csv")
  
  # Check if VPD table already exists
  if (file.exists(VPD_tbl_path)){
    message(cli::col_blue(paste("\nVPD CSV File exists:\n", VPD_tbl_path)))
    next
  }
  message(paste0("\n-- Ceating VPD table for AOI: ", AreaName))
  VPD_tbl_path <- file.path(root_folder, VPD_tbl_path)
  ...
  # Fetch VPD data from ER5 and produce dataframe per Feho
  VPD_Feho = data.frame()
  for (i in 1:nrow(FeHo_tbl)) {
    ...
    # Calculate time range for retrieving ER5 VPD data
    ...
    # Get the desired extent for raster VPD
    feat_fh <- vFireIn %>% 
      filter(FeHo == {{fhGet}})
    ...
    temp <- paste0("era5-land-VPD_RAW-", stmp_tt,".grib")
    if (!file.exists(paste(dir_path, temp, sep = "/"))){
      message(paste("Store to: \n", temp))
      request <- list(
      ...
      f1 = paste(dir_path, temp, sep = "/")
      r <- terra::rast(f1)
    }
    
    # process VPD data
    r = project(r, crs(feat_fh))
    Feho_VPD_extract = data.frame()
    for (ti in 1:(nlyr(r)/2)){
      ...
      # Calculate VPD direction and speed from vectors u & v
      Tp_d <- r[[ti*2 - 1]]-273.15
      Tp   <- r[[ti*2]]-273.15
      VPD <- 0.6109*exp(17.625*Tp/(Tp+243.04)) - 0.6109*exp(17.625*Tp_d/(Tp_d+243.04))
      VPD2 <- 0.61078*exp(17.269*Tp/(Tp+237.3)) - 0.61078*exp(17.269*Tp_d/(Tp_d+237.3))
      
      VPD <- VPD %>% rename(value = 1)
      # 1. Use centroid only
      cent_VPD_extract <- terra::extract(VPD, centroids(feat_fh))
      if (nrow(na.omit(cent_VPD_extract)) == 0){ 
        # if centroids are not inside valid rasters
        # 2. Use polygon and get touched pixels
        cent_VPD_extract <- terra::extract(VPD, feat_fh, touches = T)
      }
      
      # Clean FeHo VPD
      cent_VPD_extract <- na.omit(cent_VPD_extract)
      cent_VPD_extract <- cent_VPD_extract %>% 
        mutate(Feho = terra::time(r[[ti*2-1]]))
      
      Feho_VPD_extract <- rbind(Feho_VPD_extract,
                                 cent_VPD_extract)
    }
    print(Feho_VPD_extract)
    # Average all collected hourly VPD information within the FeHo interval
    tp_feho <- data.frame(VPD_kPa = mean(Feho_VPD_extract$value),
                          FeHo_V = fhGet)
    VPD_Feho <- rbind(VPD_Feho, tp_feho)
    rm(tp_feho)
  ...
  VPD_Feho <- VPD_Feho %>% 
    mutate(codi_hora = seq(nrow(VPD_Feho)), .before = 1)
  write.csv(VPD_Feho,
            file = VPD_tbl_path,
            row.names = FALSE)
  message(paste0("\n-- END of Writing VPD table for AOI: ", AreaName, "-------- \n"))
}
